...
class ArkeselSMSDeviceMixin:
    def setUp(self):
        try:
            ...
            self.device = alice.arkeselsmsdevice_set.create(number='+233269192964')
            self.device2 = bob.arkeselsmsdevice_set.create(number='+233247895684')


@override_settings(
    OTP_ARKESEL_NO_DELIVERY=True,
    OTP_ARKESEL_CHALLENGE_MESSAGE='{token}',
    OTP_ARKESEL_THROTTLE_FACTOR=0,
)
class TestArkeselSMSDevice(ArkeselSMSDeviceMixin, TestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_barely_made_it(self):
        ...
        with freeze_time() as frozen_time:
            token = self.device.generate_challenge()
            frozen_time.tick(delta=(settings.OTP_ARKESEL_TOKEN_VALIDITY - 1))
            ok = self.device.verify_token(token)
    ...
    @override_settings(
        ...
        OTP_ARKESEL_TOKEN_TEMPLATE='Token is {token}',
    )
    def test_format(self):
        with mock.patch('core.models.ArkeselSMSDevice._deliver_token', self._deliver_token):
...
class ThrottlingTestCase(ArkeselSMSDeviceMixin, ThrottlingTestMixin, TestCase):
